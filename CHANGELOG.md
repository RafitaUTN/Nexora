...
### Resumen
Primera versión estable de DocuMind Desktop: escaneo e indexación de documentos (PDF/Word/Excel/imágenes) con OCR, búsqueda FTS5 local y overlay global, IA opcional (clasificación, resumen y Q&A con citas), etiquetas, respaldos, automatizaciones, multiusuario con roles, licencias online, actualizaciones automáticas y sincronización entre dispositivos con Supabase y RLS por usuario.

### Añadido
- Resumen y Q&A con IA (dominio): `SummarizeService` y `QaService` con caché por hash (`ai_cache`), presupuesto de tokens, JSON estricto y RAG sobre FTS5 (5 candidatos por consulta, citas con `{documentId, filename, score}`, fragmentos acotados). Canales `ai:summarize|ai:qa` (rol editor) con auditoría; UI: sección «Resumen» en el detalle del documento y página `/qa` con citas enlazadas.
- Auto-sync en segundo plano: primer disparo a los 5 s tras arrancar y luego cada 15 min; emite `sync:status` al renderer para invalidar la sección de sincronización.
- Supabase Auth por usuario: `SupabaseAuthClient` (login/refresh/signUp contra GoTrue con `fetch`), sesión cifrada en el `SecretStore` (kind `sync`), refresco automático del token y `SupabaseSyncStore` autenticado que escribe `user_id` por fila. Canales `sync:signUp|sync:signOut` (admin) y UI de cuenta (correo/contraseña, conectar/crear/desconectar) en Ajustes.
- RLS por `auth.uid()` en el proyecto remoto: migración `rls_user_scoped_sync` añade `user_id uuid not null default auth.uid()` + índice en las 4 tablas `sync_*` y reemplaza las políticas permisivas `anon` por `sync_*_own` (`using/with check user_id = auth.uid()`).
